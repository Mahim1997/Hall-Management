...
-- ON DELETE OF A ROOM_HISTORY ROW ... ROOM ROW GETS MINUS 1 OF "current no of residents"
BEGIN
      EXECUTE IMMEDIATE 'DROP trigger trig_Room_History_Delete';
  EXCEPTION
      WHEN OTHERS THEN NULL;
END;
/
...
CREATE OR REPLACE TRIGGER trig_Room_History_Delete
BEFORE DELETE ON TABLE_ROOM_HISTORY 
FOR EACH ROW
...
BEGIN
	...
	SET TABLE_STUDENT.TYPE = 'resident' WHERE TABLE_STUDENT.ID = :NEW.STUDENT_ID;
...
AFTER UPDATE OF END_DATE ON TABLE_ROOM_HISTORY 
FOR EACH ROW
DECLARE
...
BEGIN
      EXECUTE IMMEDIATE 'DROP trigger trig_application_accepted';
  EXCEPTION
      WHEN OTHERS THEN NULL;
END;
/
...
AFTER UPDATE OF STATUS ON TABLE_APPLICATION 
FOR EACH ROW

BEGIN

	IF :new.CURRENT_ROOMID is not null then
		-- removing from old room
		UPDATE TABLE_ROOM_HISTORY
		SET TABLE_ROOM_HISTORY.END_DATE=CURRENT_DATE
		WHERE TABLE_ROOM_HISTORY.HALL_ID=:new.HALL_ID and TABLE_ROOM_HISTORY.ROOM_NO=:new.CURRENT_ROOMID and
		TABLE_ROOM_HISTORY.STUDENT_ID=:new.STUDENT_ID and TABLE_ROOM_HISTORY.END_DATE is null;
	ELSE
